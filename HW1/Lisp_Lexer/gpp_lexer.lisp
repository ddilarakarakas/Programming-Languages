...
(defun writeFile ()
    (with-open-file (stream "parsed_lisp.txt" :direction :output)
        (dotimes (m (list-length outFileToken))
            (format stream (nth m outFileToken))
            (terpri stream)
        )
    )
)
